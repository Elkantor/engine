...
*           - void DrawRectangleGradientEx(RL_Rectangle rec, Color col1, Color col2, Color col3, Color col4); // -- GuiColorPicker()
*
...
*           - void SetShapesTexture(Texture2D tex, RL_Rectangle rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
...
static void SetShapesTexture(Texture2D tex, RL_Rectangle rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
...
int GuiGetState(void) { return (int)guiState; }
...
{
    if (!guiStyleLoaded) GuiLoadStyleDefault();
    guiStyle[control*(RAYGUI_MAX_PROPS_BASE + RAYGUI_MAX_PROPS_EXTENDED) + property] = (unsigned int)value;

    // Default properties are propagated to all controls
    if ((control == 0) && (property < RAYGUI_MAX_PROPS_BASE))
    {
        for (int i = 1; i < RAYGUI_MAX_CONTROLS; i++) guiStyle[i*(RAYGUI_MAX_PROPS_BASE + RAYGUI_MAX_PROPS_EXTENDED) + property] = (unsigned int)value;
    }
}
...
int GuiWindowBox(RL_Rectangle bounds, const char *title)
{
    ...
    if (bounds.height < (float)statusBarHeight*2.0f) bounds.height = (float)statusBarHeight*2.0f;

    RL_Rectangle windowPanel = { bounds.x, bounds.y + (float)statusBarHeight - 1, bounds.width, bounds.height - (float)statusBarHeight + 1 };
    RL_Rectangle closeButtonRec = { statusBar.x + statusBar.width - (float)GuiGetStyle(STATUSBAR, BORDER_WIDTH) - 20.f,
                                 statusBar.y + (float)statusBarHeight/2.0f - 18.0f/2.0f, 18, 18 };
...
int GuiGroupBox(RL_Rectangle bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(RAYGUI_CLITERAL(RL_Rectangle){ bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, GetColor((unsigned int)GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(RL_Rectangle){ bounds.x, bounds.y + bounds.height - 1, bounds.width, RAYGUI_GROUPBOX_LINE_THICK }, 0, BLANK, GetColor((unsigned int)GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(RL_Rectangle){ bounds.x + bounds.width - 1, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, GetColor((unsigned int)GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
...
int GuiScrollPanel(RL_Rectangle bounds, const char *text, RL_Rectangle content, Vector2 *scroll, RL_Rectangle *view)
{
    ...
    if (!hasHorizontalScrollBar) hasHorizontalScrollBar = (hasVerticalScrollBar && (content.width > (bounds.width - 2.f*(float)GuiGetStyle(DEFAULT, BORDER_WIDTH) - (float)GuiGetStyle(LISTVIEW, SCROLLBAR_WIDTH))))? true : false;
    if (!hasVerticalScrollBar) hasVerticalScrollBar = (hasHorizontalScrollBar && (content.height > (bounds.height - 2.f*(float)GuiGetStyle(DEFAULT, BORDER_WIDTH) - (float)GuiGetStyle(LISTVIEW, SCROLLBAR_WIDTH))))? true : false;
    ...
    RL_Rectangle horizontalScrollBar = {
        (float)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (float)bounds.x + verticalScrollBarWidth : (float)bounds.x) + (float)GuiGetStyle(DEFAULT, BORDER_WIDTH),
        (float)bounds.y + bounds.height - horizontalScrollBarWidth - (float)GuiGetStyle(DEFAULT, BORDER_WIDTH),
        (float)bounds.width - verticalScrollBarWidth - 2.f*(float)GuiGetStyle(DEFAULT, BORDER_WIDTH),
        (float)horizontalScrollBarWidth
    };
    RL_Rectangle verticalScrollBar = {
        (float)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (float)bounds.x + (float)GuiGetStyle(DEFAULT, BORDER_WIDTH) : (float)bounds.x + bounds.width - (float)verticalScrollBarWidth - (float)GuiGetStyle(DEFAULT, BORDER_WIDTH)),
        (float)bounds.y + (float)GuiGetStyle(DEFAULT, BORDER_WIDTH),
        (float)verticalScrollBarWidth,
        (float)bounds.height - horizontalScrollBarWidth - 2.f * (float)GuiGetStyle(DEFAULT, BORDER_WIDTH)
    };
    ...
    *view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)?
                RAYGUI_CLITERAL(RL_Rectangle){ bounds.x + (float)verticalScrollBarWidth + (float)GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + (float)GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2.f * (float)GuiGetStyle(DEFAULT, BORDER_WIDTH) - (float)verticalScrollBarWidth, bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - (float)horizontalScrollBarWidth } :
                RAYGUI_CLITERAL(RL_Rectangle){ bounds.x + (float)GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + (float)GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2.f * (float)GuiGetStyle(DEFAULT, BORDER_WIDTH) - (float)verticalScrollBarWidth, bounds.height - 2.f * (float)GuiGetStyle(DEFAULT, BORDER_WIDTH) - (float)horizontalScrollBarWidth };
    ...
    float horizontalMax = hasHorizontalScrollBar? content.width - bounds.width + (float)verticalScrollBarWidth + (float)GuiGetStyle(DEFAULT, BORDER_WIDTH) - (((float)GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (float)verticalScrollBarWidth : 0) : (float)-GuiGetStyle(DEFAULT, BORDER_WIDTH);
    ...
    GuiDrawRectangle(bounds, 0, BLANK, GetColor((unsigned int)GuiGetStyle(DEFAULT, BACKGROUND_COLOR)));        // Draw background
    ...
    if (hasHorizontalScrollBar)
    {
        ...
        GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE, (int)(((bounds.width - 2.f * (float)GuiGetStyle(DEFAULT, BORDER_WIDTH) - (float)verticalScrollBarWidth)/(int)content.width)*((int)bounds.width - 2.f * (float)GuiGetStyle(DEFAULT, BORDER_WIDTH) - (float)verticalScrollBarWidth)));
...
int GuiSliderPro(RL_Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue, int sliderWidth)
{
    ...
    if (state == STATE_NORMAL) GuiDrawRectangle(slider, 0, BLANK, GetColor(GuiGetStyle(SLIDER, BORDER_COLOR_PRESSED)));
    else if (state == STATE_FOCUSED) GuiDrawRectangle(slider, 0, BLANK, GetColor(GuiGetStyle(SLIDER, BORDER_COLOR_PRESSED)));
    else if (state == STATE_PRESSED) GuiDrawRectangle(slider, 0, BLANK, GetColor(GuiGetStyle(SLIDER, BORDER_COLOR_PRESSED)));
    ...
    if (textLeft != NULL)
    {
        ...
        textBounds.height = (float)GuiGetStyle(DEFAULT, TEXT_SIZE);
        textBounds.x = bounds.x + bounds.width/2 - textBounds.width/2 - GuiGetStyle(SLIDER, TEXT_PADDING);
        textBounds.y = bounds.y + bounds.height/2 - GuiGetStyle(DEFAULT, TEXT_SIZE)/2;
...
void GuiWindowFloating(Vector2* position, Vector2* size, bool* minimized, bool* moving, bool* resizing, void (*draw_content)(Vector2, Vector2, WorldStruct*), Vector2 content_size, Vector2* scroll, WorldStruct* _world, const char* title) {
#if !defined(RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT)
#define RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT 24
#endif
...
#define RAYGUI_WINDOW_CLOSEBUTTON_SIZE 18
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && !*moving && !*resizing) {
        ...
        RL_Rectangle title_collision_rect = { position->x, position->y, size->x - (RAYGUI_WINDOW_CLOSEBUTTON_SIZE + close_title_size_delta_half), RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT };
        RL_Rectangle resize_collision_rect = { position->x + size->x - 20, position->y + size->y - 20, 20, 20 };
    ...
    if (*minimized) {
        GuiStatusBar((RL_Rectangle) { position->x, position->y, size->x, RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT }, title);

        if (GuiButton((RL_Rectangle) {
        ...
        *minimized = GuiWindowBox((RL_Rectangle) { position->x, position->y, size->x, size->y }, title);

        // scissor and draw content within a scroll panel
        if (draw_content != NULL) {
            RL_Rectangle scissor = { 0 };
            GuiScrollPanel((RL_Rectangle) { position->x, position->y + RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT, size->x, size->y - RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT },
                NULL,
                (RL_Rectangle) {
            ...
            draw_content(*position, *scroll, _world);
